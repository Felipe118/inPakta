   ...
   <section class="container-fluid">
       <div class="row justify-content-center mt-5">
           <div class="col-6">
               ...
               <h3 style="font-size: 26px;" class="mb-4">Formulário de edição de empresas</h3>
               <form action=""  @submit.prevent="editForm">
                   <div class="form-group mb-3">
                       <label for="" class="form-label">Nome:</label>
                       <input type="text"  placeholder="Nome" class=" input form-control" v-model="empresa.nome">
                       <span class="text-danger">{{ errors.nome }}</span>
                   </div>
                   <div class="form-group mb-3">
                       <label for="" class="form-label">E-mail:</label>
                       <input type="email" placeholder="E-mail" class=" input form-control" v-model="empresa.email">
                       <span class="text-danger">{{ errors.email }}</span>
                   </div>
                   <div class="form-group mb-5">
                       <label for="" class="form-label">Site:</label>
                       <input type="text" placeholder="Site" class="input form-control" v-model="empresa.website">
...
<script>
   ...
   export default {
       ...
       methods: {
           editForm(){
              this.$inertia.post('/editar-empresa',this.empresa, {
                   onSuccess: (resp) => {
                       this.message = resp
                       this.sucess = true
                       this.messageForm()
